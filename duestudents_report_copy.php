...
if(isset($sms))
{
	...
	if($status==0)
	{
	echo "<script>window.location='send_sms_duestudents.php?s=search&class=$class&section=$section&range=$range&r1=$r1&r2=$r2'</script>";
	}
	else
	{
                                    ...
                                    <thead>
                                        <tr>
											...
											if(isset($search))
											{
												if($class!="")
												{
													$qs1 = mysqli_query($con,"select * from assign_fee_class where class_id='$class'");
													$rs1 = mysqli_fetch_array($qs1);
													$feeamt = $rs1['fee_header_amount'];
													$feeamtarr = explode(',',$feeamt);
													$feestr = $rs1['fee_header_id'];
