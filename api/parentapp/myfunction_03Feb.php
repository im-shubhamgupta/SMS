...
function fees($regno)
{
	...
	if($row)
	{
		...
		$q1 = mysqli_query($con,"select * from student_wise_fees where student_id='$stuid'");
		$r1 = mysqli_fetch_assoc($q1);
		$feeid = $r1['fee_header_id'];	
		$feeidarr = explode(',',$feeid);
		$feeamt = $r1['fee_amount'];
		...
		$array = array_combine($feeidarr,$feeamtarr);
		
		foreach($array as $key=>$value)
		{
			@$temp = array();
			$q2 = mysqli_query($con,"select * from fee_header where fee_header_id='$key'");
...
function eventcalendar($fromdate,$todate,$classid,$sectionid)
{
	...
	$q1=mysqli_query($con,"select * from events where class_id='$classid' AND section_id='$sectionid' 
	AND from_date>='$nfromdate' AND to_date<='$ntotdate'");
