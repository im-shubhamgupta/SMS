		...
		switch($_GET['apicall']){
			...
			case 'uploadpic':
				...
				if(isset($_FILES['pic']['name']) && isset($_POST['tags'])){
					...
					try{
						...
						$stmt = $con->prepare("INSERT INTO student_notifications (photos, message) VALUES (?,?)");
			...
			case 'getpics':
				...
				while($stmt->fetch()){
					...
					$temp['image'] = 'http://localhost/school_management/uploads/'.$image;
					$temp['tags'] = $tags; 
