...
$r=mysqli_fetch_array($q);
$fhid=$r['fee_header_id'];
$amt1=$r['received_amount'];
$amt2=explode(',',$amt1);
$amt3 = 0;
$amt4 = 0;
foreach($amt2 as $k)
...
$prevamt=$r['previous_amount'];
$amt4=$r['transport_amount'];
$dueamt=$r['due_amount'];
$tamt = $amt3 + $prevamt + $amt4;

$ptid=$r['payment_type_id'];
$p1 = mysqli_query($con,"select * from payment_type where payment_type_id='$ptid'");
$p2=mysqli_fetch_array($p1);
$paidby = $p2['payment_type_name'];

$cno=$r['payment_detail'];
$issby=$r['issued_by'];
$issdt=$r['issue_date'];
...
mysqli_query($con,"update students set due='$ndue' where student_id='$stuid'");
mysqli_query($con,"update student_wise_fees set due_amount='$ndue' where student_id='$stuid'");

$q3 = "update student_due_fees set status=4, total_amount='$tamt', action_type='$actiontype', loginuser='$loginuser', reason='$reason', action_date=now() where student_due_fee_id='$id'";
if(mysqli_query($con,$q3))
{
	$action = $sname." School Fees of Rs ".$amt3." has been deleted."; 
	...
	$msg = "Dear Mr. ".$fname.",%0aYour ".$gen." ".$sname." School Fees of Rs ".$amt3." has been discarded. Please contact school administration and collect the amount.%0a From,%0a".$sclname;
...
echo "<script>window.location='dashboard.php?option=view_payment&stuid=$stuid'</script>";
