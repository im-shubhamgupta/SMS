...
if(isset($_POST['search']))
{
	...
	if($stock!="" and $fromdt!="" and $todt!="")
	{
	$query = "SELECT * FROM `issue_order` WHERE stock_type_id='$stock' && issued_date between '$fromdt' AND '$todt'";
	$search_result = filterTable($query);	
	}
	else if($stock=="" and $fromdt!="" and $todt!="")
	{
	$query = "SELECT * FROM `issue_order` WHERE issued_date between '$fromdt' AND '$todt'";
	$search_result = filterTable($query);	
	}	
	else if($stock=="" and $fromdt=="" and $todt=="")
	{
	$query = "SELECT * FROM `issue_order`";
	$search_result = filterTable($query);	
	}	
                ...
                <div class="row">
                    <div class="col-md-12">
						...
						<div class="row" style="margin-top:20px;">
								...
								?>
								<option value="<?php echo $rows['stock_type_id']; ?>"><?php echo $rows['stock_type_name']; ?>
								</option>
								<?php } ?>	
								</select>
								</div>
						...
						<div class="card">
							...
							<div class="card-footer">
								<a href="export_issueorder_report_excel.php?fromdt=<?php echo $fromdt;?>&todt=<?php echo $todt;?>&stock=<?php echo $stock;?>" class="btn btn-success btn-sm">
